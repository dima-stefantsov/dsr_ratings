...
$GLOBALS['rating_calculators']['main_v3'] = [
    'description' => '
        DSR rating v3
    ...
    'main' => 'dsr_main_v3__main',
    ];

function dsr_main_v3__main(&$teams) {
    dsr_main_v3__uncompress_rating($teams);
    dsr_main_v3__update_ratings($teams);
    dsr_main_v3__compress_rating($teams);
}

function dsr_main_v3__uncompress_rating(&$teams) {
...
function dsr_main_v3__compress_rating(&$teams) {
    foreach ($teams as &$team) {
        foreach ($team as &$player) {
...
function dsr_main_v3__update_ratings(&$teams) {
    $full_mirror_multiplier = dsr_main_v3__get_full_mirror_multiplier($teams);
    ...
    foreach ($teams as &$team) {
        foreach ($team as &$player) {
            $status_multiplier = dsr_main_v3__get_status_multiplier($player['status']);
...
function dsr_main_v3__get_full_mirror_multiplier(&$teams) {
    $races0 = [];
...
function dsr_main_v3__get_status_multiplier($status) {
    if ($status === 'win') {
        return 1;
    }
