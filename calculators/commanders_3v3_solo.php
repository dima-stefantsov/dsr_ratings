...
$GLOBALS['rating_calculators']['commanders_3v3_solo'] = [
    'description' => '
        solo commanders_3v3

        same as <a href="/rating/commanders_3v3">commanders_3v3</a>, but only for solo games with random teammates
    ...
    'main' => 'dsr_commanders_3v3_solo__main',
    ];

function dsr_commanders_3v3_solo__main(&$teams) {
    $teams_with_solo_team_rating = dsr_commanders_3v3_solo__get_teams_with_solo_team_rating($teams);

    $GLOBALS['rating_calculators']['commanders_3v3']['main']($teams_with_solo_team_rating);

    dsr_commanders_3v3_solo__assign_ratings($teams, $teams_with_solo_team_rating);
}

function dsr_commanders_3v3_solo__get_teams_with_solo_team_rating(&$teams) {
    $ratings_solo_team = &$GLOBALS['rating_calculators']['commanders_3v3_solo']['data']['ratings_solo_team'];
    ...
    foreach ($teams_with_solo_team_rating as &$team) {
        foreach ($team as &$player) {
            ...
            $player['rating'] =
                ...
                $GLOBALS['rating_calculators']['commanders_3v3_solo']['default_rating'];
            $GLOBALS['rating_calculators']['main_current']['data']['players_games_count'][$player['player_id']] = &$GLOBALS['rating_calculators']['commanders_3v3_solo']['data']['solo_team_players_games_count'][$player['is_solo']][$player['player_id']];
        }
    }
...
function dsr_commanders_3v3_solo__assign_ratings(&$teams, &$teams_with_solo_team_rating) {
    $ratings_solo_team = &$GLOBALS['rating_calculators']['commanders_3v3_solo']['data']['ratings_solo_team'];
